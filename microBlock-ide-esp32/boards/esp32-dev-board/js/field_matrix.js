...
// validator.
CustomFields.FieldMatrix = function(opt_value, opt_validator) {

  // The turtle field contains an object as its value, so we need to compile
  // the parameters into an object.
  var value = opt_value || "\x00\x00\x00\x00\x00\x00\x00\x00";
...
CustomFields.FieldMatrix.fromJson = function(options) {
  ...
  return new CustomFields.FieldMatrix(options['value']);
};
...
CustomFields.FieldMatrix.prototype.text2byte = (text) => {
  let bytes = [ ];
  bytes = text.split("\\x");
...
CustomFields.FieldMatrix.prototype.byte2text = (bytes) => {
...
CustomFields.FieldMatrix.prototype.updateEditable = function() {
  if (!this.fieldGroup_) {
    // Not initialized yet.
    return;
  }
...
CustomFields.FieldMatrix.prototype.getText = function() {
  return "<CUSTOM>";
};
...
// local validator.
CustomFields.FieldMatrix.prototype.doValueInvalid_ = function(invalidValue) {
...
CustomFields.FieldMatrix.prototype.renderEditor_ = function(firsttimes) {
...
let collection = [
  ...
  "\\x08\\x04\\x02\\x04\\x08\\x10\\x20\\x40",
  "\\x00\\x42\\x24\\x18\\x18\\x24\\x42\\x00",
  "\\x08\\x1c\\x2a\\x49\\x08\\x08\\x08\\x08",
  "\\x08\\x08\\x08\\x08\\x49\\x2a\\x1c\\x08",
  "\\x38\\x44\\x42\\x21\\x21\\x42\\x44\\x38",
];
...
CustomFields.FieldMatrix.prototype.showEditor_ = function() {
  var html = "";
  html += `<div class="FieldMatrixDropdown">`;
  html += `<div class="menu">`;
  ...
  this.editor_.querySelector("button.fill").addEventListener("click", function() {
    thisObj.setValue("\\xFF\\xFF\\xFF\\xFF\\xFF\\xFF\\xFF\\xFF");
  });
  ...
  this.editor_.querySelector("button.clear").addEventListener("click", function() {
    thisObj.setValue("\\x00\\x00\\x00\\x00\\x00\\x00\\x00\\x00");
  });
...
CustomFields.FieldMatrix.prototype.dropdownDispose_ = function() {
  delete this.editor_;
};

// Updates the field's colour based on the colour of the block. Called by
// block.applyColour.
CustomFields.FieldMatrix.prototype.applyColour = function() {
...
CustomFields.FieldMatrix.prototype.createImageDotMatrix_ = function(value) {
